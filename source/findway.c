...
void Dijkstra(int start, int end, int way[], int *count)//由于最后存储是逆序的，所以实际找的是终点到起点的最短路径
{
    int i,j;
    // int count=0;
    // const int vexnum = 10;
...
//进入函数的条件是鼠标在地图区域点击，并且点击的位置为白色利用int  getpixel(int x,int y)
//node:路口结点
...
// unsigned int try;
    ...
    while (1)
    {
        ...
        if (mx >= 1 && mx <= 767 && my >= 1 && my <= 767 && button  )
        {
            ...
            if (mousepress(mx,my) != 0)//检测是否在道路上
            {
                ...
                else//起点和终点在同一个区域
                {
                    ...
                    {
                        rentmove(&car_position,placenow.x,placenow.y,placeto.x,placeto.y,x,y,1,energe);//向上走
                    }
                    else if (placenow.y<placeto.y)
                    {
                        rentmove(&car_position,placenow.x,placenow.y,placeto.x,placeto.y,x,y,2,energe);//向下走
                    }
                    else if (placenow.x>placeto.x)
                    {
                        rentmove(&car_position,placenow.x,placenow.y,placeto.x,placeto.y,x,y,3,energe);//向左走
                    }
                    else if (placenow.x<placeto.x)
                    {
                        rentmove(&car_position,placenow.x,placenow.y,placeto.x,placeto.y,x,y,4,energe);//向右走
                    }
                }
